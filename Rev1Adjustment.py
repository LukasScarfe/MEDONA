...
oxylene = solution(1.00E-10, 875.23, 30.10, 7.5e4, 3.7, 8.715E-3, 1.85E-3, 2.56)
...
def init_drop_size(Q): 
    ...
    k = oxylene.elec_cond #electrical conductivity
    rho = oxylene.liq_mass_dens #liquid mass density
    gamma = oxylene.surf_tens #liquid-air interfacial tension
...
def K_e():
    ...
    D_diff = oxylene.diff_coeff #mass diffusivity
    rho = oxylene.liq_mass_dens #again the liquid mass density... this should be input?? List of parameters can be always input?
    rho_g = oxylene.vap_dens #ITS SOLVENT VAPOUR DENSITY in my infinite wisdom I forgot to write down what this variable is,,,, time to go paper hunting
    P_rat = oxylene.press_rat #pressure ratio
    Ke = 8*D_diff*(rho_g/rho)*(P_rat) #final equation
    return Ke

def current(Q):
    #current of droplet
    gamma = oxylene.surf_tens #surface tension
    k = oxylene.elec_cond #electrical Conductivity
    #Q = 1E-3 #flow rate
    kappa = oxylene.dielec_const #dielectric constant
    scalConst = 1
...
def height_for_varying_volt(d_0, q_0, e_tim, V):
    ...
    mu = oxylene.air_visc #viscosity of air
...
# for i in range(len(flow_rates)):
    ...
    # path1 = 'C:/Users/jenni/Documents/University/FINAL YEAR!!!!/ENGPHYS 4A06 Capstone/Rev0Graphs/flow rate/' + fname1
...
# mpl.title(title)
# mpl.savefig('C:/Users/jenni/Documents/University/FINAL YEAR!!!!/ENGPHYS 4A06 Capstone/Rev0Graphs/DropletSizePlot') 
# mpl.show()
